...
where hgap_id = $id";
    ...
    int hgap_id = Convert.ToInt32(btnet.Util.sanitize_integer(Request["revpathid"]));
	sql = sql.Replace("$id", Convert.ToString(hgap_id));

	DataRow dr = btnet.DbUtil.get_datarow(sql);

	// check if user has permission for this bug
    int permission_level = btnet.Bug.get_bug_permission_level((int)dr["hgrev_bug"], security);
    ...
    blame_text = btnet.VersionControl.hg_blame(repo, path, revision);
