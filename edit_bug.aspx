...
<script language="C#" runat="server">
    ...
    void prepare_for_insert()
    {
        ...
        titl.InnerText = btnet.Util.get_setting("AppTitle", "TDSS Bugs Tracker") + " - Create ";
    ...
    void prepare_a_bunch_of_links_for_update()
    {
        ...
        if (permission_level == Security.PERMISSION_ALL)
        {
            string clone_link = "<a class=warn href=\"javascript:clone()\" "
                + " title='Create a copy of this item'><img src=images/paste_plain.png border=0 align=top>&nbsp;create copy</a>";
        ...
        {
            string attachment_link = "<img src=images/attach.gif align=top>&nbsp;<a href=\"javascript:open_popup_window('add_attachment.aspx','add attachment ',"
                + Convert.ToString(id)
        ...
        if (!security.user.is_guest)
        {
            if (permission_level != Security.PERMISSION_READONLY)
            {
                string send_email_link = "<a href='javascript:send_email("
                    + Convert.ToString(id)
                    + ")' title='Send an email about this item'><img src=images/email_edit.png border=0 align=top>&nbsp;send email</a>";
        ...
        if (permission_level != Security.PERMISSION_READONLY)
        {
            string subscribers_link = "<a target=_blank href=view_subscribers.aspx?id="
                + Convert.ToString(id)
                + " title='View users who have subscribed to email notifications for this item'><img src=images/telephone_edit.png border=0 align=top>&nbsp;subscribers</a>";
        ...
        if (btnet.Util.get_setting("EnableRelationships", "0") == "1")
        {
            ...
            string relationships_link = "<a target=_blank href=relationships.aspx?bgid="
                + Convert.ToString(id)
                + " title='Create a relationship between this item and another item'><img src=images/database_link.png border=0 align=top>&nbsp;relationships(<span id=relationship_cnt>" + relationship_cnt + "</span>)</a>";
        ...
        if (btnet.Util.get_setting("EnableSubversionIntegration", "0") == "1")
        {
            ...
            string svn_revisions_link = "<a target=_blank href=svn_view_revisions.aspx?id="
                + Convert.ToString(id)
            + " title='View Subversion svn_revisions related to this item'><img src=images/svn.png border=0 align=top>&nbsp;svn revisions(" + revision_cnt + ")</a>";
        ...
        if (btnet.Util.get_setting("EnableGitIntegration", "0") == "1")
        {
            ...
            string git_commits_link = "<a target=_blank href=git_view_revisions.aspx?id="
                + Convert.ToString(id)
            + " title='View git git_commits related to this item'><img src=images/git.png border=0 align=top>&nbsp;git commits(" + revision_cnt + ")</a>";
        ...
        if (btnet.Util.get_setting("EnableMercurialIntegration", "0") == "1")
        {
            ...
            string hg_revisions_link = "<a target=_blank href=hg_view_revisions.aspx?id="
                + Convert.ToString(id)
            + " title='View mercurial git_hg_revisions related to this item'><img src=images/hg.png border=0 align=top>&nbsp;hg revisions(" + revision_cnt + ")</a>";
        ...
        if (security.user.is_admin || security.user.can_view_tasks)
        {
            if (btnet.Util.get_setting("EnableTasks", "0") == "1")
            {
                ...
                string tasks_link = "<a target=_blank href=tasks_frame.aspx?bugid="
                    + Convert.ToString(id)
                + " title='View sub-tasks/time-tracking entries related to this item'><img src=images/clock.png border=0 align=top>&nbsp;tasks/time(<span id=task_cnt>" + task_cnt + "</span>)</a>";
        ...
        print.InnerHtml = "<a target=_blank href=print_bug.aspx?id="
            + Convert.ToString(id)
            + " title='Display this item in a printer-friendly format'><img src=images/printer.png border=0 align=top>&nbsp;print</a>";
        ...
        if (!security.user.is_guest)
        {
            ...
            || security.user.can_merge_bugs)
            {
                string merge_bug_link = "<a href=merge_bug.aspx?id="
                    + Convert.ToString(id)
                    + " title='Merge this item and another item together'><img src=images/database_refresh.png border=0 align=top>&nbsp;merge</a>";
        ...
        if (!security.user.is_guest)
        {
            ...
            || security.user.can_delete_bug)
            {
                string delete_bug_link = "<a href=delete_bug.aspx?id="
                    + Convert.ToString(id)
                    + " title='Delete this item'><img src=images/delete.png border=0 align=top>&nbsp;delete</a>";
        ...
        if (btnet.Util.get_setting("CustomBugLinkLabel", "") != "")
        {
            string custom_bug_link = "<a href="
                ...
                + "?bugid="
                + Convert.ToString(id)
                + "><img src=images/brick.png border=0 align=top>&nbsp;"
    ...
    void get_comment_text_from_control()
    {
        ...
        {
            comment_formated = btnet.Util.strip_dangerous_tags(txtediter.Text);
            comment_search = btnet.Util.strip_html(txtediter.Text);
            commentType = "text/html";
        }
        else
        {
            comment_formated = HttpUtility.HtmlDecode(txtediter.Text);
        ...
        if (notification_email_enabled)
        {
            ...
            else
            {
                ...
                string subscription_link = "<a id='notifications' title='Get or stop getting email notifications about changes to this item.'"
                    ...
                    + Convert.ToString(id)
                    + ")'><img src=images/telephone.png border=0 align=top>&nbsp;<span id='get_stop_notifications'>";
            ...
            if (this_bug_found)
            {
                if (prev_bug != 0)
                {
                    prev_next_link =
                        ...
                        + "><img src=images/arrow_up.png border=0 align=top>prev</a>";
                }
                else
                {
                ...
                if (next_bug != 0)
                {
                    prev_next_link +=
                        ...
                        + ">next<img src=images/arrow_down.png border=0 align=top></a>";
...
where us_id = @us_id";
        ...
        foreach (DataRow drcc in ds_custom_cols.Tables[0].Rows)
        {
            ...
            || field_permission_level == Security.PERMISSION_READONLY)
            {
                ...
                else
                {
                    ...
                    if (datatype == "datetime")
                    {
                        text = btnet.Util.format_db_date_and_time(text);
                    }
                }
    ...
    void display_project_specific_custom_fields()
    {
        ...
        && project.SelectedItem.Value != "0")
        {
            ...
            '|v'+CAST(CONVERT(INTEGER,SUBSTRING(ISNULL(pj_custom_dropdown_values1,'v61001'),LEN(ISNULL(pj_custom_dropdown_values1,'v61001'))-4,5))+1 AS varchar(5)) pj_custom_addversion
                ...
                for (int i = 1; i < 4; i++)
                {
                    if ((int)project_dr["pj_enable_custom_dropdown" + Convert.ToString(i)] == 1)
                    {
                        ...
                        else
                        {
                            ...
                            string[] options = btnet.Util.split_project_version(
                            ...
                            for (int j = 0; j < options.Length; j++)
                            {
                                Response.Write("<option value=\"" + "v" + options[j].ToString() + "\"");

                                //if (options[j] == selected_value)
                                if (HttpUtility.HtmlDecode(options[j].ToString()) == selected_value)
                                {
                                    Response.Write(" selected ");
                                }
                                Response.Write(">");
                                Response.Write("v" + options[j].ToString());
                            }
...
<head>
    ...
    <%--<script src="jquery/ckediter/ckeditor.js" type="text/javascript"></script>--%>
    <link href="jquery/ckediter/contents.css" rel="stylesheet" type="text/css" />
...
{
           CKEDITOR.addCss('.cke_editable p { margin: 0 !important;line-height: 1.4;}');
            CKEDITOR.addCss('.cmt_text p { margin: 0 !important; }');
...
<body onload='on_body_load()' onunload='on_body_unload()'>
    ...
    <div class="align">
        ...
        <a class="warn" href="edit_bug.aspx?id=0">
            <img src="images/add.png" border="0" align="top">&nbsp;Add New
            <% Response.Write(btnet.Util.get_setting("SingularBugLabel", "bug")); %></a>
        &nbsp;&nbsp;&nbsp;&nbsp;
        <% } %>
        <span id="prev_next" runat="server">&nbsp;</span>
        <!--ADDED BY SATHISH KUMAR A.D ON 12/31/2018 -->
        &nbsp;<input type="button" value="Back" onclick="location.replace('bugs.aspx');"
            class="menubtn1" />
        <br>
        <br>
                    ...
                    <td nowrap valign="top">
                        ...
                        <div id="bugform_div">
                            ...
                            <div style="text-align: center;">
                                <input runat="server" class="btn" type="submit" id="submit_button2" onclick="on_user_hit_submit()"
                                    value="Update">
                            </div>
                            <% } %>
                            ...
                            <table width="90%" border="0" cellpadding="3" cellspacing="0">
                                <tr>
                                    <td nowrap>
                                        ...
                                        <!--COMMENTED BY SATHISH KUMAR A.D ON 12/31/2018 -->
                            ...
                            <table border="0" cellpadding="0" cellspacing="3" width="98%">
                                <tr>
                                    <td nowrap>
                                        ...
                                        <CKEditor:CKEditorControl ID="txtediter" runat="server" BaseHref="ckeditor/" Height="80"
                                            RemovePlugins="Save,Print,Preview,Find,About,Maximize,ShowBlocks" AutoGrowMinHeight="180"> </CKEditor:CKEditorControl>
                                        <tr>
                                            <td nowrap>
                                                <asp:CheckBox runat="server" class="cb" ID="internal_only" Checked="true"/>
                                                <%--<span  runat="server" id="internal_only_label">Comment visible to internal users only</span>--%>
                                                <label  runat="server" id="internal_only_label" for="internal_only">Comment visible to internal users only</label>
                                                <tr>
                                                    <td nowrap align="left">
                                                        ...
                                                        <tr>
                                                            <td nowrap align="center">
                                                                <input runat="server" class="btn" type="submit" id="submit_button" onclick="on_user_hit_submit()"
                                                                    value="Update">
                            </table>
