...
void Page_Load(Object sender, EventArgs e)
{
	...
	titl.InnerText = btnet.Util.get_setting("AppTitle","TDSS Bugs Tracker") + " - "
		+ "send email";
	...
	if (!IsPostBack)
	{
		...
		if (string_bp_id != null)
		{
			...
			if (dr["us_signature"].ToString() != "")
			{
				...
				{
				    body.Text += "<br><br><br>";
					body.Text += dr["us_signature"].ToString().Replace("\r\n", "<br>");
					body.Text += "<br><br><br>";
				}
				else
				{
					body.Text += "\n\n\n";
					body.Text += dr["us_signature"].ToString();
					body.Text += "\n\n\n";
				}
			}
			...
			if (Request["quote"] != null)
			{
				...
				if (dr["bp_type"].ToString() == "received")
				{
					...
					{
					    body.Text += "<br><br><br>";
					    body.Text += "&#62;From: " + dr["bp_email_from"].ToString().Replace("<", "&#60;").Replace(">", "&#62;") + "<br>";
					}
					else
					{
						body.Text += "\n\n\n";
						body.Text += ">From: " + dr["bp_email_from"] + "\n";
					}
				}
				...
				for (int i = 0; i < lines.Length; i++)
				{
					if (i < 4 && (lines[i].IndexOf("To:") == 0 || lines[i].IndexOf("Cc:") == 0))
					{
						...
						if (security.user.use_fckeditor)
						{
                            body.Text += "&#62;" + lines[i].Replace("<", "&#60;").Replace(">", "&#62;") + "<br>";
						}
						else
						{
							body.Text += ">" + lines[i] + "\n";
					...
					else if (next_line_is_date)
					{
						...
						if (security.user.use_fckeditor)
						{
							body.Text += "&#62;Date: " + Convert.ToString(dr["bp_date"]) + "<br>&#62;<br>";
						}
						else
						{
							body.Text += ">Date: " + Convert.ToString(dr["bp_date"]) + "\n>\n";
						}
					}
					else
					{
...
update bugs set
	...
	if (security.user.use_fckeditor)
	{
		...
		adjusted_body += btnet.Util.strip_dangerous_tags(body.Text);
	...
	else
	{
    	...
    	adjusted_body += HttpUtility.HtmlDecode(body.Text);
	...
	else
	{
		...
		{
			body_text = HttpUtility.HtmlDecode(body.Text);
			//body_text = body_text.Replace("\n","\r\n");
...
function my_on_load()
{
CKEDITOR.addCss('#cke_1_contents div {height: 387px;}');
	<%
	if (security.user.use_fckeditor)	
	{
...
<form class=frm runat="server" enctype="multipart/form-data">
    ...
    <CKEditor:CKEditorControl ID="body" runat="server" BaseHref="ckeditor/" Height="274px"
           RemovePlugins="Save,Print,Preview,Find,About,Maximize,ShowBlocks"  AutoGrowMinHeight="100"></CKEditor:CKEditorControl>
	</td>
	</tr>
