...
void Page_Load(Object sender, EventArgs e)
{
    ...
    security = new btnet.Security();
    security.check_security(HttpContext.Current, btnet.Security.ANY_USER_OK);
...
order by svnrev_revision desc, svnap_path";
    ...
    int svnap_id = Convert.ToInt32(btnet.Util.sanitize_integer(Request["revpathid"]));
    ...
    int permission_level = btnet.Bug.get_bug_permission_level((int)dr["svnrev_bug"], security);
    if (permission_level == btnet.Security.PERMISSION_NONE)
    {
    ...
    raw_text = btnet.VersionControl.svn_cat(repo, path, revision);
    ...
    blame_text = btnet.VersionControl.svn_blame(repo, path, revision);
